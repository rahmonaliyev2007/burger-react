...
import burgerImg from '../Components/Images/pizza.png'
...
function Pizza() {
  ...
  const handleEditBurger = (id) => {
    navigate(`/edit/${id}`);
    setCurrectCategory('pizzas');
  };


  const handleDeleteBurger = (id) => {
    axios.delete(`http://localhost:5200/pizzas/${id}`)
      .then(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5200/pizzas').then(res => setBurgers(res.data))
  }, [])

  return (
    <>
      <div className='flex justify-between max-abu items-end'>
        <h2 className={`text-4xl ${theme === 'dark' ? 'text-white' : "text-black"}`}>{t('p')}</h2>
        <button className='p-4 bg-yashil text-white hover:bg-yashilhover' onClick={() => handleAddBurger('pizzas')} >{t('add_p')} </button>
      </div>
          ...
          return (
            <div key={product.id} className=' max-w-[350px] card mt-9 max500:mt-4 cursor-pointer max500:flex max500:gap-4 max500:justify-between max500:items-center'>
              <div className='card-top relative min-w-32 rounded-2xl overflow-hidden'>
                ...
                <div className='card-overlay max1020:text-base max500:text-xs text-center w-full h-full flex justify-center items-center absolute bg-yashilhover top-0 opacity-1 text-2xl text-white opacity-0 transition-all duration-300'>{t('add_cart')}</div>
