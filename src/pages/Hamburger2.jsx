...
function Hamburger() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5200/burgers').then(res => setBurgers(res.data))
  }, [])
  ...
  const handleEditBurger = (id) => {
    navigate(`/edit/${id}`);
    setCurrectCategory('burgers');
  };


  const handleDeleteBurger = (id) => {
    axios.delete(`http://localhost:5200/burgers/${id}`)
      .then(() => {
  ...
  return (
    <>
      <div className='flex justify-between max-abu items-end'>
        <h2 className={`text-4xl ${theme === 'dark' ? 'text-white' : "text-black"}`}>{t('b')}</h2>
        <button className='p-4 bg-yashil text-white hover:bg-yashilhover' onClick={() => handleAddBurger('burgers')} >{t('add_b')} </button>
      </div>
