...
import burgerImg from '../Components/Images/juice.png'
...
function Juice() {
  ...
  const handleEditBurger = (id) => {
    navigate(`/edit/${id}`);
    setCurrectCategory('juice');
  };


  const handleDeleteBurger = (id) => {
    axios.delete(`http://localhost:5200/juice/${id}`)
      .then(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5200/juice').then(res => setBurgers(res.data))
  }, [])

  return (
    <>
      <div className='flex justify-between max-abu items-end'>
        <h2 className={`text-4xl ${theme === 'dark' ? 'text-white' : "text-black"}`}>{t('j')}</h2>
        <button className='p-4 bg-yashil text-white hover:bg-yashilhover' onClick={() => handleAddBurger('juice')} >{t('add_j')} </button>
      </div>
