...
import burgerImg from '../Components/Images/cola.png'
...
function Soda() {
  ...
  const handleEditBurger = (id) => {
    navigate(`/edit/${id}`);
    setCurrectCategory('soda');
  };


  const handleDeleteBurger = (id) => {
    axios.delete(`http://localhost:5200/soda/${id}`)
      .then(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5200/soda').then(res => setBurgers(res.data))
  }, [])

  return (
    <>
      <div className='flex justify-between max-abu items-end'>
        <h2 className={`text-4xl ${theme === 'dark' ? 'text-white' : "text-black"}`}>{t('n')}</h2>
        <button className='p-4 bg-yashil text-white hover:bg-yashilhover' onClick={() => handleAddBurger('soda')} >{t('add_n')} </button>
      </div>
